...
#############################################################################
# Post-Deploy Script for Host Inventory Ephemeral Environment
#############################################################################
# This script runs the post-deployment steps on an already-deployed
# environment (skips namespace reservation and deploy_services):
# 1. Login to Ephemeral cluster (if needed)
# 2. Detect current namespace
# 3. Setup port-forwarding for services
# 4. Retrieve database credentials and Unleash token
# 5. Update .env file with database credentials, Unleash token, and Kessel auth
# 6. Update /etc/hosts with Kafka bootstrap server
# 7. Save outputs to tmp directory
#############################################################################
...
# Function to detect current namespace
detect_namespace() {
    log "Detecting current namespace..."

    NAMESPACE=$(oc project -q 2>/dev/null || true)

    if [[ -z "$NAMESPACE" || "$NAMESPACE" == "default" ]]; then
        log_error "No active namespace found. Please ensure services are already deployed."
        log_error "You can switch to your namespace with: oc project <namespace>"
        exit 1
    fi
...
display_summary() {
    ...
    echo ""
    echo "Updated Files:"
    if [[ "$ENV_FILE_UPDATED" == "true" ]]; then
        echo -e "  ${GREEN}✓${NC} $HBI_DIR/.env (database credentials, Unleash token, Kessel auth)"
    else
        echo -e "  ${RED}✗${NC} $HBI_DIR/.env (update failed)"
    fi
    if [[ "$ETC_HOSTS_UPDATED" == "true" ]]; then
        echo -e "  ${GREEN}✓${NC} /etc/hosts (Kafka bootstrap server)"
    else
        echo -e "  ${RED}✗${NC} /etc/hosts (update failed or skipped)"
    fi
    echo ""
...
main() {
    echo ""
    echo "========================================================================"
    echo "  Post-Deploy Script for Host Inventory Ephemeral Environment"
    echo "========================================================================"
    echo ""
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            --help)
                ...
                echo ""
                echo "Runs the post-deployment steps on an already-deployed environment."
                echo "Assumes services are already deployed and a namespace is active."
                echo ""
                echo "Options:"
